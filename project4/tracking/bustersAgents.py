        ...
        for Dist in livingGhostPositionDistributions:
            ...
            for Pos, Prob in Dist.items():
                if Prob > bestProb:
                    bestProb = Prob
                    bestPos = Pos
            posGhosts.append(bestPos)
        
        minDistance = self.distancer.getDistance(pacmanPosition, posGhosts[0])
        minAction = "Stop"
        for action in legal:
            pos = Actions.getSuccessor(pacmanPosition, action)
            distances = [self.distancer.getDistance(ghostPos, pos) for ghostPos in posGhosts]
